...
namespace Hyperletter.Dispatcher {
    public class HandlerDispatcher : AbstractDispatcher, IHandlerDispatcher {
        private readonly IHandlerFactory _handlerFactory;
        private readonly DictionaryList<Type, Registration> _registry = new DictionaryList<Type, Registration>();

        public HandlerDispatcher(IAbstractHyperSocket hyperSocket, IHandlerFactory handlerFactory, ITransportSerializer serializer) : base(hyperSocket, serializer) {
            _handlerFactory = handlerFactory;
        }

        public void Register<TMessage, THandler>() where THandler : IHandler {
        ...
        private class Registration<TMessage, THandler> : Registration where THandler : IHandler {
            private readonly IHandlerFactory _handlerFactory;
            ...
            public override void Invoke(ILetter letter, Type concreteType) {
                ...
                var handler = _handlerFactory.CreateHandler<THandler>(message);
                handler.Execute();
            }
        }
